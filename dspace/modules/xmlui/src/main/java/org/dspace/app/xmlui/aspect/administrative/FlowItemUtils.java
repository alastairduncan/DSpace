...
import org.dspace.storage.bitstore.BigFileBitstreamStorage;
...
public class FlowItemUtils {
	 ...
	 * 
	 * If the identifier was unable to be resolved to an item then the
	 * "identifier" field is placed in error.
	 * 
	 * @param context
	 *            The current DSpace context.
	 * @param identifier
	 *            An Internal ID or a handle
	...
	public static FlowResult resolveItemIdentifier(Context context, String identifier) throws SQLException {
		...
		// Check whether it's a handle or internal id (by check ing if it has a
		// slash inthe string)
	...
	/**
	 * Process the request parameters to update the item's metadata and remove
	 * any selected bitstreams.
	 * 
	 ...
	 * entry (i.e schema_element_qualifier). The value and language paramaters
	 * are user inputed fields. If the optional parameter "remove_X" is given
	 * then the metadata value is removed.
	 * 
	 * To support AJAX operations on this page an aditional parameter is
	 * considered, the "scope". The scope is the set of metadata entries that
	 * are being updated during this request. It the metadata name,
	 * schema_element_qualifier, only fields that have this name are considered!
	 * If all fields are to be considered then scope should be set to "*".
	 * 
	 * When creating an AJAX query include all the name_X, value_X, language_X,
	 * and remove_X for the fields in the set, and then set the scope parameter
	 ...
	 *            The DSpace context
	 * @param itemID
	 *            The id of the to-be-withdrawn item.
	...
	/**
	 * Reinstate the specified item, this method assumes that the action has
	 * been confirmed.
	 * 
	 * @param context
	 *            The DSpace context
	 * @param itemID
	 *            The id of the to-be-reinstated item.
	 * @return A result object
	 */
	...
	/**
	 * Make the specified item Private, this method assumes that the action has
	 * been confirmed.
	 *
	 * @param context
	 *            The DSpace context
	 ...
	 *            The DSpace context
	 * @param itemID
	 *            The id of the to-be-moved item.
	 * @param collectionID
	 *            The id of the destination collection.
	 * @param inherit
	 *            Whether to inherit the policies of the destination collection
	 * @return A result object
	 */
	...
	/**
	 * Permanently delete the specified item, this method assumes that the
		...
		Item item = Item.find(context, itemID);
		 ...
		 for(String key: files.keySet()){
			 ...
			 // There is a bundle already, just add bitstream 
				 String filePath = BigFileBitstreamStorage.getIntermediatePath(key) + key;
	 ...
	 * description will be used to create a new bitstream. If the format needs
	 * to be adjusted then they will need to access the edit bitstream form
	 * after it has been uploaded.
	 * 
	 * @param context
	 *            The DSpace content
	 * @param itemID
	 *            The item to add a new bitstream too
	 * @param request
	 *            The request.
	 * @return A flow result
	 */
	 ...
	 *            The DSpace content
	 * @param itemID
	 *            The item to which the bitstream belongs
	 * @param bitstreamID
	 ...
	 * @param userFormat
	 *            Any user supplied formats.
	 * @return A flow result object.
	 */
	public static FlowResult processEditBitstream(Context context, int itemID, int bitstreamID, String bitstreamName, String primary, String description, int formatID, String userFormat,
			Request request) throws SQLException, AuthorizeException {
		FlowResult result = new FlowResult();
		result.setContinue(false);
		...
		if (bundles != null && bundles.length > 0) {
			if (bitstreamID == bundles[0].getPrimaryBitstreamID()) {
				...
				if ("no".equals(primary)) {
					// However the user has removed this bitstream as a primary
					// bitstream.
	...
	/**
	 * Delete the given bitstreams from the bundle and item. If there are no
	 * more bitstreams left in a bundle then also remove it.
	 * 
	 * @param context
	 *            Current dspace content
	 * @param itemID
	 *            The item id from which to remove bitstreams
	 * @param bitstreamIDs
	 *            A bundleID slash bitstreamID pair of bitstreams to be removed.
	 * @return A flow result
	 */
	 ...
	 *
	 */
	public static FlowResult processCurateItem(Context context, int itemID, Request request) throws AuthorizeException, IOException, SQLException, Exception {
	 ...
	 */
	public static FlowResult processQueueItem(Context context, int itemID, Request request) throws AuthorizeException, IOException, SQLException, Exception {
		String task = request.getParameter("curate_task");
		Curator curator = FlowCurationUtils.getCurator(task);
		...
		if (item != null) {
			...
			try {
				...
				// no-op (the Curator should have already logged any error that
				// occurred)
			}
		}
	 ...
	 * represent the schema, element, and qualifier. You are guaranteed that if
	 * no qualifier was supplied the third entry is null.
	 * 
	 * @param name
