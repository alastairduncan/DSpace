...
public class BitstreamReader extends AbstractReader implements Recyclable {
	...
	/**
	 * TEMP file for citation PDF. We will save here, so we can delete the temp
	 * file when done.
	 */
	...
	public void setup(SourceResolver resolver, Map objectModel, String src, Parameters par) throws ProcessingException, SAXException, IOException {
		...
		try {
			...
			// Intercepting views to the original bitstream to instead show a
			// citation altered version of the object
			// We need to check if this resource falls under the
			// "show watermarked alternative" umbrella.
			// At which time we will not return the "bitstream", but will
			...
			new DSpace().getEventService().fireEvent(
					new UsageEvent(UsageEvent.Action.VIEW, ObjectModelHelper.getRequest(objectModel), ContextUtil.obtainContext(ObjectModelHelper.getRequest(objectModel)), bitstream));
	...
	public void generate() throws IOException, SAXException, ProcessingException {
		...
		try {
			if (byteRange != null) {
				...
				copyRange(this.bitstreamInputStream, out, start, end);
			...
			try {
				// Close the bitstream input stream so that we don't leak a
				// file
				// descriptor
				...
				// Closing the stream threw an IOException but do we want
				// this
