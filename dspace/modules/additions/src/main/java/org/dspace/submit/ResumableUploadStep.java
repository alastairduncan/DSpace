...
public class ResumableUploadStep extends AbstractProcessingStep {
	...
	public int doProcessing(Context context, HttpServletRequest request, HttpServletResponse response, SubmissionInfo subInfo) throws ServletException, IOException, SQLException, AuthorizeException {
		...
		if ((contentType != null) && (contentType.indexOf("multipart/form-data") != -1)) {
			...
			log.debug("doProcessing: processUploadFile status: " + status);
			// if error occurred, return immediately
			if (status != STATUS_COMPLETE) {
				log.debug("doProcessing: status not complete process upload file failed!!");
				return status;
			}
		}
		...
		if (buttonPressed.startsWith(PROGRESS_BAR_PREFIX) || buttonPressed.startsWith(PREVIOUS_BUTTON)) {
			...
			if (fileRequired && !item.hasUploadedFiles()) {
				log.debug("doProcessing: no files error a file is required but none were found ");
				return STATUS_NO_FILES_ERROR;
			} else {
				log.debug("doProcessing: status complete file required and and at least one found");
		...
		if (request.getParameter("bitstream_id") != null) {
			if (buttonPressed.equals(CANCEL_EDIT_BUTTON)) {
				...
				log.debug("doProcessing: status edit complete ");
				...
				log.debug("doProcessing: saved bitstream to submission info object");
			}
		...
		if (buttonPressed.equalsIgnoreCase("submit_remove_selected")) {
			...
			if (request.getParameter("remove") != null) {
				...
				for (int i = 0; i < removeIDs.length; i++) {
					...
					if (status != STATUS_COMPLETE) {
						...
						log.debug("doProcessing: status not complete submit remove selected problem ");
						return status;
					}
				}
			...
			// if error occurred, return immediately
			if (status != STATUS_COMPLETE) {
				log.debug("doProcessing: status not complete remove problem ");
				return status;
			}
		...
		if (fileDescription != null && fileDescription.length() > 0) {
			...
			// if error occurred, return immediately
			if (status != STATUS_COMPLETE) {
				log.debug("doProcessing: status not complete file description problem");
				return status;
			}
		}
		...
		if (formatTypeID >= 0 || (formatDesc != null && formatDesc.length() > 0)) {
			...
			// if error occurred, return immediately
			if (status != STATUS_COMPLETE) {
				log.debug("doProcessing: status not complete file format problem ");
				return status;
			}
		}
		...
		if (request.getParameter("primary_bitstream_id") != null) {
			...
			if (bundles.length > 0) {
				bundles[0].setPrimaryBitstreamID(Integer.valueOf(request.getParameter("primary_bitstream_id")).intValue());
				bundles[0].update();
			}
		}
		...
		context.commit();
		log.debug("doProcessing: status now complete all committed");
		return STATUS_COMPLETE;
	}
	 ...
	 *         (if STATUS_COMPLETE or 0 is returned, no errors occurred!)
	 */
	public int processUploadFile(Context context, HttpServletRequest request, HttpServletResponse response, SubmissionInfo subInfo) throws ServletException, IOException, SQLException,
			AuthorizeException {
		...
		for (int i = 0; i < parameterValues.length; i++) {
			log.debug("processUploadFile: value: " + parameterValues[i]);
			...
			log.debug("processUploadFile: id " + itemID + " bitstreamId " + bitstreamId + " filename " + filename);

			if (subInfo == null) {
				log.error("processUploadFile: subInfo is null status integrity error!");
			...
			if (bundles.length < 1) {
				log.debug("processUploadFile: create single bitstream");
				// set bundle's name to ORIGINAL
				b = item.createBigSingleBitstream(bitstreamId, "ORIGINAL", Integer.valueOf(itemID));
				log.debug("processUploadFile: bitstream size: " + b.getSize());
				// b.setSource(n);
				// b = item.createSingleBitstream(fileInputStream, "ORIGINAL");
			} else {
				log.debug("processUploadFile: already got a bundle add the bitstrean to it");
				// we have a bundle already, just add bitstream
				b = bundles[0].createBigBitstream(bitstreamId, Integer.valueOf(itemID));
			...
			if (item.hasUploadedFiles()) {
				log.debug("processUploadFile: item has uploaded files :-)");
			} else {
				log.debug("processUploadFile: item does not have uploaded files :-( what's gone wrong?");
			}

			if ((bf != null) && (bf.isInternal())) {
				log.warn("processUploadFile: Attempt to upload file format marked as internal system use only");
			...
			if (ConfigurationManager.getBooleanProperty("submission-curation", "virus-scan")) {
				...
				if (status == Curator.CURATE_ERROR) {
					log.error("processUploadFile: virus checker unavailable");
					backoutBitstream(subInfo, b, item);
					return STATUS_VIRUS_CHECKER_UNAVAILABLE;
				} else if (status == Curator.CURATE_FAIL) {
					log.error("processUploadFile: Curator failed - virus detected");
					backoutBitstream(subInfo, b, item);
			...
			if (bf == null) {
				log.error("processUploadFile: Could not detect file format format");
	...
	/*
	 * If we created a new Bitstream but now realised there is a problem then
	 * remove it.
	 */
	 ...
	 *         (if STATUS_COMPLETE or 0 is returned, no errors occurred!)
	 */
	protected int processSaveFileFormat(Context context, HttpServletRequest request, HttpServletResponse response, SubmissionInfo subInfo) throws ServletException, IOException, SQLException,
			AuthorizeException {
	 ...
	 *         (if STATUS_COMPLETE or 0 is returned, no errors occurred!)
	 */
	protected int processSaveFileDescription(Context context, HttpServletRequest request, HttpServletResponse response, SubmissionInfo subInfo) throws ServletException, IOException, SQLException,
			AuthorizeException {
		if (subInfo.getBitstream() != null) {
