...
import org.dspace.content.BitStreamOutput;
 ...
 * 
 * <P>
 * Presently, asset stores are specified in <code>dspace.cfg</code>. Since Java
 * does not offer a way of detecting free disk space, the asset store to use for
 * new bitstreams is also specified in a configuration property. The drawbacks
 * to this are that the administrators are responsible for monitoring available
 * space in the asset stores, and DSpace (Tomcat) has to be restarted when the
 * asset store for new ('incoming') bitstreams is changed.
 * </P>
 * 
 * <P>
 ...
 * 
 * <p>
 * Cleanup integration with checker package by Nate Sarr 2006-01. N.B. The
 * dependency on the checker package isn't ideal - a Listener pattern would be
 * better but was considered overkill for the purposes of integrating the
 * checker. It would be worth re-considering a Listener pattern if another
 * package needs to be notified of BitstreamStorageManager actions.
 * </p>
 *
 * @author Peter Breton, Robert Tansley, David Little, Nathan Sarr
 * @version $Revision$
 */
public class BigFileBitstreamStorage {
	/** log4j log */
	private static Logger log = Logger.getLogger(BigFileBitstreamStorage.class);

	private static Map<Integer, BitStreamOutput> streams = new HashMap<Integer, BitStreamOutput>();
	// the metadata associated with the bitstream
	private static Map<Integer, TableRow> files = new HashMap<Integer, TableRow>();
	...
	static {
		...
		if (sAssetstoreDir != null) {
			...
			stores.add(new SRBAccount( // srb
					ConfigurationManager.getProperty("srb.host"), ConfigurationManager.getIntProperty("srb.port"), ConfigurationManager.getProperty("srb.username"), ConfigurationManager
							.getProperty("srb.password"), ConfigurationManager.getProperty("srb.homedirectory"), ConfigurationManager.getProperty("srb.mdasdomainname"), ConfigurationManager
							.getProperty("srb.defaultstorageresource"), ConfigurationManager.getProperty("srb.mcatzone")));
		} else {
		...
		for (int i = 1;; i++) { // i == 0 is default above
			...
			} else if (ConfigurationManager.getProperty("srb.host." + i) != null) { // srb
				stores.add(new SRBAccount(ConfigurationManager.getProperty("srb.host." + i), ConfigurationManager.getIntProperty("srb.port." + i), ConfigurationManager
						.getProperty("srb.username." + i), ConfigurationManager.getProperty("srb.password." + i), ConfigurationManager.getProperty("srb.homedirectory." + i), ConfigurationManager
						.getProperty("srb.mdasdomainname." + i), ConfigurationManager.getProperty("srb.defaultstorageresource." + i), ConfigurationManager.getProperty("srb.mcatzone." + i)));
			} else {
	 ...
	 */
	public static int store(Context context, String internalId, int itemID) throws SQLException, IOException {

		TableRow bitstream = files.get(itemID);
		
		String intermediatePath = getIntermediatePath(internalId);
		
		String path = intermediatePath + internalId;
		long fileSize = 0L;
		...
		try {
			...
			bitstream.setColumn("internal_id", internalId);
			files.put(itemID, bitstream);
	 ...
	 *            The current context
	 * @param assetstore
	 *            The assetstore number for the bitstream to be registered
	 * @param bitstreamPath
	 *            The relative path of the bitstream to be registered. The path
	 *            is relative to the path of ths assetstore.
	...
	public static int register(Context context, int assetstore, String bitstreamPath) throws SQLException, IOException {
		...
		int bitstreamId = bitstream.getIntColumn("bitstream_id");
		if (log.isDebugEnabled()) {
			log.debug("Stored bitstream " + bitstreamId + " in file " + file.getAbsolutePath());
		}
		return bitstreamId;
	}
	 ...
	 *
	 * @param internalId
	 *            the value of the internal_id column
	 ...
	 * be undone.
	 * 
	 * @param deleteDbRecords
	 *            if true deletes the database records otherwise it only deletes
	 *            the files and directories in the assetstore
	 * @exception IOException
	...
	public static void cleanup(boolean deleteDbRecords, boolean verbose) throws SQLException, IOException {
		...
		try {
			...
			List<TableRow> storage = DatabaseManager.queryTable(context, "Bitstream", myQuery).toList();
			...
			for (Iterator<TableRow> iterator = storage.iterator(); iterator.hasNext();) {
				...
				TableRow duplicateBitRow = DatabaseManager.querySingleTable(context, "Bitstream", "SELECT * FROM Bitstream WHERE internal_id = ? AND bitstream_id <> ?",
						row.getStringColumn("internal_id"), bid);
	 ...
	 */
	public static GeneralFile getFile(TableRow bitstream) throws IOException {
