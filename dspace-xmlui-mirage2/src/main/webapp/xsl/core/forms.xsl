...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <!-- add bitstream -->
    <xsl:template match="dri:field[@id='aspect.submission.StepTransformer.field.file']">
        <xsl:call-template name="resumable-upload"></xsl:call-template>
    </xsl:template>

    <!-- update bitstream -->
    <xsl:template match="dri:field[@id='aspect.administrative.item.AddBitstreamForm.field.file']">
        <xsl:call-template name="resumable-upload"></xsl:call-template>
    </xsl:template>

    <!-- Resumable box and javascript for large uploads -->
    <xsl:template name="resumable-upload" priority="2">
        ...
        <div class="resumable-drop" ondragenter="jQuery(this).addClass('resumable-dragover');" ondragend="jQuery(this).removeClass('resumable-dragover');" ondrop="jQuery(this).removeClass('resumable-dragover');">
            Drop files here to upload or <a href="#" class="resumable-browse">select from your computer</a>
        </div>
        ...
        <div class="resumable-progress">
            <div class="row">
                ...
                <div class="col-sm-1">
                    ...
                    <div class="progress-pause" nowrap="nowrap">
                        <a href="#" onclick="r.upload(); return(false);" class="progress-resume-link">
                            <img src="{concat($theme-path,'images/resume.png')}" title="Resume upload" />
                        </a>
                        <a href="#" onclick="r.pause(); return(false);" class="progress-pause-link">
                            <img src="{concat($theme-path,'images/pause.png')}" title="Pause upload" />
                        </a>
                    </div>
                </div>
            </div>
...
$(function() {
    ...
    $("#aspect_administrative_item_AddBitstreamForm_field_submit_upload").hide();
});
...
if (!r.support) {
    ...
    r.on('complete', function(){
        ...
        $("#aspect_submission_StepTransformer_field_submit_upload").show();
        $("#aspect_administrative_item_AddBitstreamForm_field_submit_upload").show();
    });
    r.on('fileSuccess', function(file,message){
        // Reflect that the file upload has completed
        $('.resumable-file-'+file.uniqueIdentifier+' .resumable-file-progress').html('(completed)');

        // write out the bitstreamids in the form
        $('#aspect_submission_StepTransformer_div_submit-upload')
            .append('<input type="hidden" name="file-name-'+message+'" value="'+file.fileName+'" />')
            .append('<input type="hidden" name="bitstream-id" value="'+message+'" />');
        $('#aspect_administrative_item_AddBitstreamForm_div_add-bitstream')
            .append('<input type="hidden" name="file-name-'+message+'" value="'+file.fileName+'" />')
            .append('<input type="hidden" name="bitstream-id" value="'+message+'" />');
    });
