    	...
    	<div class="resumable-error"> 
            Your browser, unfortunately, is not supported by Resumable.js. The library requires support for <a href="http://www.w3.org/TR/FileAPI/">the HTML5 File API</a> along with <a href="http://www.w3.org/TR/FileAPI/#normalization-of-params">file slicing</a>. 
    			...
    			<div class="resumable-drop" ondragenter="jQuery(this).addClass('resumable-dragover');"
    				ondragend="jQuery(this).removeClass('resumable-dragover');" ondrop="jQuery(this).removeClass('resumable-dragover');">
            ...
            // Show a place for dropping/selecting files
             ...
             r.on('pause', function(){
              ...
              // write out the bitstreamids in the form
				...
				// different form if its submission or edit
				
				var elementExists = document.getElementById("aspect_submission_StepTransformer_div_submit-upload");
				
				if(elementExists != null){

	   				$('#aspect_submission_StepTransformer_div_submit-upload').append('<input type="hidden" name="file-name-'+message+'" value="'+file.fileName+'" />');
	                $('#aspect_submission_StepTransformer_div_submit-upload').append('<input type="hidden" name="bitstream-id" value="'+message+'" />');
              	}else{
