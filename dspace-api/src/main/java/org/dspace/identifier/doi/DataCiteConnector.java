...
public class DataCiteConnector implements DOIConnector {
	 ...
	 * dependency injection.
	 * 
	 * @param DATACITE_SCHEME
	 *            Probably https or http.
	 */
	@Required
	 ...
	 * 
	 * @param DATACITE_HOST
	 *            Hostname to connect to register DOIs (f.e. test.datacite.org).
	 */
	@Required
	 ...
	 * dependency injection.
	 * 
	 * @param DATACITE_DOI_PATH
	 *            Path to register DOIs, f.e. /doi.
	 */
	@Required
	 ...
	 * dependency injection.
	 * 
	 * @param DATACITE_METADATA_PATH
	 *            Path to register metadata, f.e. /mds.
	 */
	@Required
	...
	protected void prepareXwalk() {
		...
		if (this.xwalk == null) {
			throw new RuntimeException("Can't find crosswalk '" + CROSSWALK_NAME + "'!");
		}
	}
	...
	protected String getPassword() {
		if (null == this.PASSWORD) {
			...
			if (null == this.PASSWORD) {
				throw new RuntimeException("Unable to load password from " + "configuration. Cannot find property " + CFG_PASSWORD + ".");
			}
		}
	...
	public boolean isDOIReserved(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
		...
		// 200 -> reserved
		// if (200 && dso != null) -> compare url (out of response-content) with
		// dso
		case (200): {
		...
		default: {
			log.warn("While checking if the DOI {} is registered, we got a " + "http status code {} and the message \"{}\".",
					new String[] { doi, Integer.toString(resp.statusCode), resp.getContent() });
			throw new DOIIdentifierException("Unable to parse an answer from " + "DataCite API. Please have a look into DSpace logs.", DOIIdentifierException.BAD_ANSWER);
		}
		}
	}
	...
	public boolean isDOIRegistered(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
		...
		case (200): {
			...
			if (null == doiUrl) {
				...
				throw new DOIIdentifierException("Received a http status code 200 without a response content.", DOIIdentifierException.BAD_ANSWER);
			}
			...
			if (null == dsoUrl) {
				...
				log.error("The HandleManager was unable to find the handle " + "of a DSpaceObject in the database!?! " + "Type: {} ID: {}", dso.getTypeText(), dso.getID());
				throw new RuntimeException("The HandleManager was unable to " + "find the handle of a DSpaceObject in the database!");
			}
	...
	public void deleteDOI(Context context, String doi) throws DOIIdentifierException {
		...
		// 404 "Not Found" means DOI is neither reserved nor registered.
		case (404): {
			log.error("DOI {} is at least reserved, but a delete request " + "told us that it is unknown!", doi);
			return;
		}
		// Catch all other http status code in case we forgot one.
		default: {
			log.warn("While deleting metadata of DOI {}, we got a " + "http status code {} and the message \"{}\".", new String[] { doi, Integer.toString(resp.statusCode), resp.getContent() });
			throw new DOIIdentifierException("Unable to parse an answer from " + "DataCite API. Please have a look into DSpace logs.", DOIIdentifierException.BAD_ANSWER);
		}
		}
	}
	...
	public void reserveDOI(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
		...
		if (!this.xwalk.canDisseminate(dso)) {
			...
			throw new DOIIdentifierException("Cannot disseminate " + dso.getTypeText() + "/" + dso.getID() + " using crosswalk " + this.CROSSWALK_NAME + ".", DOIIdentifierException.CONVERSION_ERROR);
		}
		...
		try {
			...
			log.error("Caught an AuthorizeException while disseminating DSO " + "with type " + dso.getType() + " and ID " + dso.getID() + ". Giving up to reserve DOI " + doi + ".", ae);
			throw new DOIIdentifierException("AuthorizeException occurred while " + "converting " + dso.getTypeText() + "/" + dso.getID() + " using crosswalk " + this.CROSSWALK_NAME + ".", ae,
					DOIIdentifierException.CONVERSION_ERROR);
		} catch (CrosswalkException ce) {
			log.error("Caught an CrosswalkException while reserving a DOI (" + doi + ") for DSO with type " + dso.getType() + " and ID " + dso.getID() + ". Won't reserve the doi.", ce);
			throw new DOIIdentifierException("CrosswalkException occurred while " + "converting " + dso.getTypeText() + "/" + dso.getID() + " using crosswalk " + this.CROSSWALK_NAME + ".", ce,
					DOIIdentifierException.CONVERSION_ERROR);
		...
		if (null == metadataDOI) {
			...
			throw new DOIIdentifierException("DSO with type " + dso.getTypeText() + " and id " + dso.getID() + " already has DOI " + metadataDOI + ". Won't reserve DOI " + doi + " for it.");
		}
		...
		case (400): {
			...
			throw new DOIIdentifierException("Unable to reserve DOI " + doi + ". Please inform your administrator or take a look " + " into the log files.", DOIIdentifierException.BAD_REQUEST);
		}
		// Catch all other http status code in case we forgot one.
		default: {
			log.warn("While reserving the DOI {}, we got a http status code " + "{} and the message \"{}\".", new String[] { doi, Integer.toString(resp.statusCode), resp.getContent() });
			throw new DOIIdentifierException("Unable to parse an answer from " + "DataCite API. Please have a look into DSpace logs.", DOIIdentifierException.BAD_ANSWER);
		}
		}
	}
	...
	public void registerDOI(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
		...
		if (this.isDOIRegistered(context, doi)) {
			...
			if (!this.isDOIReserved(context, dso, doi)) {
				...
				if (this.isDOIReserved(context, doi)) {
					log.warn("Trying to register DOI {}, that is reserved for " + "another dso.", doi);
					throw new DOIIdentifierException("Trying to register a DOI " + "that is reserved for another object.", DOIIdentifierException.DOI_ALREADY_EXISTS);
				}

				// the DOIIdentifierProvider should catch and handle this
				throw new DOIIdentifierException("You need to reserve a DOI " + "before you can register it.", DOIIdentifierException.RESERVE_FIRST);
			}
		}
		...
		case (400): {
			...
			throw new DOIIdentifierException("Currently we cannot register " + "DOIs. Please inform the administrator or take a look " + " in the DSpace log file.", DOIIdentifierException.BAD_REQUEST);
		}
		// 412 Precondition failed: DOI was not reserved before registration!
		case (412): {
			log.error("We tried to register a DOI {} that was not reserved " + "before! The registration agency told us: {}.", doi, resp.getContent());
			throw new DOIIdentifierException("There was an error in handling " + "of DOIs. The DOI we wanted to register had not been " + "reserved in advance. Please contact the administrator "
					+ "or take a look in DSpace log file.", DOIIdentifierException.RESERVE_FIRST);
		}
		// Catch all other http status code in case we forgot one.
		default: {
			log.warn("While registration of DOI {}, we got a http status code " + "{} and the message \"{}\".", new String[] { doi, Integer.toString(resp.statusCode), resp.getContent() });
			throw new DOIIdentifierException("Unable to parse an answer from " + "DataCite API. Please have a look into DSpace logs.", DOIIdentifierException.BAD_ANSWER);
		}
		}
	}
	...
	public void updateMetadata(Context context, DSpaceObject dso, String doi) throws DOIIdentifierException {
		...
		if (!this.isDOIReserved(context, dso, doi) && this.isDOIReserved(context, doi)) {
			...
			throw new DOIIdentifierException("Trying to update metadta for " + "a DOI that is reserved for another object.", DOIIdentifierException.DOI_ALREADY_EXISTS);
		}
	...
	protected DataCiteResponse sendDOIPostRequest(String doi, String url) throws DOIIdentifierException {
		...
		String doipath = null;
		if (DOI_PATH.endsWith("/")) {
			doipath = DOI_PATH.substring(0, DOI_PATH.lastIndexOf("/"));
		} else {
			doipath = DOI_PATH;
		}
		uribuilder.setScheme(SCHEME).setHost(HOST).setPath(doipath);
		...
		try {
			...
			log.error("The URL was {}.", SCHEME + "://" + HOST + DOI_PATH + "/" + doi.substring(DOI.SCHEME.length()));
	...
	protected DataCiteResponse sendMetadataPostRequest(String doi, Element metadataRoot) throws DOIIdentifierException {
		...
		format.setEncoding("UTF-8");
		XMLOutputter xout = new XMLOutputter(format);
		String out = xout.outputString(new Document(metadataRoot));
		out = out.replace("^M", "");

		return sendMetadataPostRequest(doi, out);
	}
	...
	protected DataCiteResponse sendMetadataPostRequest(String doi, String metadata) throws DOIIdentifierException {
		...
		if (METADATA_PATH.endsWith("/")) {
			mpath = METADATA_PATH.substring(0, METADATA_PATH.lastIndexOf("/"));
		} else {
			mpath = METADATA_PATH;
		}

		uribuilder.setScheme(SCHEME).setHost(HOST).setPath(mpath);
		...
		try {
			...
			log.debug("sendMetadataPostRequest ************************************************");
			log.debug(metadata);
			log.debug("sendMetadataPostRequest ************************************************");
		...
		try {
			...
			switch (statusCode) {
				...
				case (401): {
					...
					throw new DOIIdentifierException("Cannot authenticate at the " + "DOI registry agency. Please check if username " + "and password are set correctly.",
				...
				case (403): {
					log.info("Managing a DOI ({}) was prohibited by the DOI " + "registration agency: {}", doi, content);
					throw new DOIIdentifierException("We can check, register or " + "reserve DOIs that belong to us only.", DOIIdentifierException.FOREIGN_DOI);
				}
				...
				case (500): {
					...
					throw new DOIIdentifierException("DataCite API has an internal error. " + "It is temporarily impossible to manage DOIs. " + "Further information can be found in DSpace log file.",
	...
	protected String extractAlternateIdentifier(Context context, String content) throws SQLException, DOIIdentifierException {
		...
		try {
			...
			throw new DOIIdentifierException("Got a JDOMException while parsing " + "a response from the DataCite API.", jde, DOIIdentifierException.BAD_ANSWER);
		}
